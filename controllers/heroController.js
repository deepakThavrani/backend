...
exports.updateHeroSection = async (req, res) => {
  try {
    const { images, mobileImages, intervalMs, mobileIntervalMs } = req.body;
    ...
    if (!heroSection) {
      heroSection = new HeroSection({
        ...
        mobileIntervalMs: mobileIntervalMs || intervalMs || 5000,
        isActive: true,
      });
    } else {
      heroSection.images = images;
      if (mobileImages) heroSection.mobileImages = mobileImages;
      if (intervalMs) heroSection.intervalMs = intervalMs;
      if (mobileIntervalMs) heroSection.mobileIntervalMs = mobileIntervalMs;
    }
